...
export class Booking {
  ...
  static async generateInvoice(bookingId, servicePrice, paymentMethod = 'CASH') {
    try {
      ...
      let providerPayout = servicePrice
      let commissionOwed = 0

      if (paymentMethod === 'PAYSTACK') {
        // Commission is deducted automatically for Paystack payments
        platformCommission = servicePrice * (process.env.PLATFORM_COMMISSION_RATE || 0.18)
        providerPayout = servicePrice - platformCommission
      } else if (paymentMethod === 'CASH') {
        // Provider owes commission to platform for cash payments
        commissionOwed = servicePrice * (process.env.PLATFORM_COMMISSION_RATE || 0.18)
