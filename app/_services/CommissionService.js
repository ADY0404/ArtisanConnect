...
export class CommissionService {
   ...
   */
  static async getCommissionRate(providerTier = 'NEW', serviceType = 'STANDARD') {
    try {
      ...
      // Use admin-managed rates if available (simplified tier system)
      const baseRates = commissionConfig?.rates || {
        STANDARD: 18.0,  // Standard rate for all providers
        PREMIUM: 15.0    // Reduced rate for premium providers
      }

      // Convert percentage to decimal
      const ratePercentage = baseRates[providerTier] || baseRates.STANDARD
      ...
      var baseRate = baseRates[providerTier] || baseRates.STANDARD
    }
   ...
   */
  static async calculateCommission(amount, providerTier = 'NEW', serviceType = 'STANDARD', paymentMethod = 'CASH') {
