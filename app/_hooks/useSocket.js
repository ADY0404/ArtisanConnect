...
export function useSocket() {
  ...
  const determineUserRole = (user) => {
    // Check if user email is in provider domain or has provider flag
    if (user.email?.includes('provider') || user.role === 'provider') {
      return 'provider'
    }
    // Check against provider database if needed
